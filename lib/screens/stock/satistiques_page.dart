...
class _StatistiquesPageState extends State<StatistiquesPage> {
  ...
  Future<void> fetchStatistics() async {
    ...
    for (var doc in querySnapshot.docs) {
      String cartName = doc['cartName'] ?? 'Unknown Cart';

      // Incrémenter le nombre d'occurrences pour chaque 'cartName'
      cartNameStats[cartName] = (cartNameStats[cartName] ?? 0) + 1;
    }
              ...
              bottomTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (double value, TitleMeta meta) {
                    final cartName = cartNameStats.keys.elementAt(value.toInt());
                    return SideTitleWidget(
                      axisSide: meta.axisSide,
                      space: 8.0, // Espacement entre les barres et les titres
                      child: Text(
                        cartName, // Affiche le nom complet sans troncature
